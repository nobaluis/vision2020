 ...
 */
void pushLbp(lbp::LBP& lbpDec, std::vector<double>& desc, cv::Mat& srcImg, cv::Mat& dest){
    lbpDec.calcLBP(srcImg);  // compute the LBP
    desc = lbpDec.calcHist().getHist(true); // compute the histogram descriptor
    std::vector<float> descf(desc.begin(), desc.end()); // cast to float vector
    dest.push_back(cv::Mat(descf, true).reshape(1,1)); // store to dest
}
 ...
 */
void computeLbp(lbp::LBP& lbpDec, std::vector<double>& desc,
    ...
    pushLbp(lbpDec, desc, srcImg, dest);
    if(augmented){
...
int main(){
    ...
    for(auto file : positives){
        computeLbp(lbp, descriptor, file, img, samples, true);
    }
    // 2.2. Compute negatives descriptors
    for(auto file : negatives){
        computeLbp(lbp, descriptor, file, img, samples, false);
    }
    ...
    for(auto file: positives_test){
        computeLbp(lbp, descriptor, file, img, test_samples, false);
    }
    ...
    for(int i = 0; i < predictions.rows; ++i){
        std::cout << "prediction at " << i << " is " << predictions.at<float>(i) << '\n';
    ...
    std::cout << "total fails: " << pos_fails+neg_fails << '\n';
    std::cout << "total pos fails: " << pos_fails << '\n';
    std::cout << "total neg fails: " << neg_fails << '\n';
